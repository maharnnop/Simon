...
const speech = [new SpeechSynthesisUtterance("Up"),
new SpeechSynthesisUtterance("Right"),
new SpeechSynthesisUtterance("Left"),
new SpeechSynthesisUtterance("Down"),
new SpeechSynthesisUtterance("Wrong")] // array voice
speech.forEach((item) =>{item.rate = 3}) //speed of speech (1-10) more to faster
...
if(JSON.parse(window.localStorage.getItem(user))===null){
  stageH =1
}else{stageH =JSON.parse(window.localStorage.getItem(user))[0]}
...
function keySelectColor(event) {
  ...
  if (indexKey >= 0) {
    ...
    } else {   speechSynthesis.speak(speech[4])// voice speech
      if(lp>0){ 
  ...
  for (var i = 0; i < window.localStorage.length; i++){
    ...
    for(let i = 0; i<scoreBoard.length; i++){
        if( scoreBoard[i][0] === text){
            ...
            break
        }else{window.localStorage.setItem(text, '[1,0,100]')}}
